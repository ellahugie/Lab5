...
``` r
# Merge the temperature level information back to the met dataset and assign it to met_merge
met_merge <- merge(met, average_temp_by_state, by = "STATE")
setDT(met_merge)

# Ensure 'temp_level' is a factor
met_merge$temp_level <- factor(met_merge$temp_level, levels = c("Low", "Mid", "High"))

# Compute statistics by temperature levels
